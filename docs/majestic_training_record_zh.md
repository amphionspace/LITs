...
当前显式选择完整保存的 21k，是为了复用中早期声学基础；20k 常规文件已不在保存集。21k/41k/61k/99k/197k 曾在同一批 48 条新域样本上做只读诊断，包含 speaker 重置前后比较。没有执行优化或迁移 A/B 训练，不能声称实验已证明 21k 最佳。旧 197k 不作为本轮固定 duration teacher。
...
完整 checkpoint 指标已按本文采集时刻导出到 [checkpoint_metrics.csv](majestic_training_record_assets/checkpoint_metrics.csv)，包含全部已完成节点及四组结果；1k 单独标为 smoke。

| Step | 中文 WavLM/CAMP | 中文 CER | 英文 WavLM/CAMP | 英文 WER | 混读 CER / 英文 WER |
|---:|---:|---:|---:|---:|---:|
| 15,000 | 0.763/0.683 | 1.14% | 0.721/0.794 | 1.45% | 1.68%/9.38% |
| 45,000 | 0.776/0.694 | 0.66% | 0.743/0.803 | 1.16% | 0.53%/8.33% |
| 100,000 | 0.780/0.695 | 0.58% | 0.750/0.810 | 0.92% | 0.62%/6.25% |
| 140,000 | 0.784/0.699 | 0.56% | 0.760/0.810 | 1.11% | 0.71%/8.33% |
| 145,000 | 0.786/0.689 | 0.58% | 0.760/0.811 | 1.30% | 0.18%/7.29% |
| 150,000 | 0.785/0.697 | 0.50% | 0.758/0.810 | 0.87% | 1.86%/10.42% |

上述表格使用相同 650 条完整评测中的目标分组。历史 140k 和 150k 曾作为试听候选：150k 中文/英文自动指标均衡，但混读 CER 高于 140k。没有一个统一的、经过校准的“最佳 checkpoint”分数，也尚未用听感确立全局最优。

用户已试听合成训练音频，认为所听例子中文自然；训练模型的中文输出仍被反馈有外国腔，45k 时未明显消失。不能由这些少量样本断言整个合成库毫无口音问题，也不能用低 CER 或较高 speaker similarity 推翻主观反馈。

中文 speech raw duration 的 Std/MAS Std：旧 197k 诊断约 0.385（旧 512 条数据），当前 5k/15k/45k/150k 约 0.614/0.683/0.648/0.663。当前没有持续朝 1 改善，平均误差下降也不等于节奏自然度提高。旧报告的数据不同，不能严格量化跨实验提升。
