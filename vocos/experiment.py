...
class VocosExp(pl.LightningModule):
    ...
    def __init__(
        ...
        mel_loss: MelSpecReconstructionLoss | None = None,
    ):
        """
        Args:
            ...
            mel_loss (MelSpecReconstructionLoss, optional): Custom mel loss. Defaults to torchaudio mel matching sample_rate.
        """
        super().__init__()
        self.save_hyperparameters(ignore=["feature_extractor", "backbone", "head", "mel_loss"])
        ...
        self.melspec_loss = mel_loss or MelSpecReconstructionLoss(sample_rate=sample_rate)

        self.train_discriminator = False
        self.base_mel_coeff = self.mel_loss_coeff = mel_loss_coeff

    def _log_mel(self, audio: torch.Tensor) -> torch.Tensor:
        mel = self.melspec_loss.mel_spec(audio.unsqueeze(0) if audio.dim() == 1 else audio)
        if mel.dim() == 3:
            mel = mel[0]
        if not getattr(self.melspec_loss, "use_hifigan_mel", False):
            mel = safe_log(mel)
        return mel
    ...
    def training_step(self, batch, batch_idx, optimizer_idx, **kwargs):
        ...
        if optimizer_idx == 1:
            ...
            if self.global_step % 1000 == 0 and self.global_rank == 0:
                ...
                with torch.no_grad():
                    mel = self._log_mel(audio_input[0])
                    mel_hat = self._log_mel(audio_hat[0])
    ...
    def validation_epoch_end(self, outputs):
        if self.global_rank == 0:
            *_, audio_in, audio_pred = outputs[0].values()
            ...
            mel_target = self._log_mel(audio_in)
            mel_hat = self._log_mel(audio_pred)
            self.logger.experiment.add_image(
                "val_mel_target",
