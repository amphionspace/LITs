...
def parse_student_t_grid(value: str | None, student_steps: int) -> List[float]:
    if value is None or not str(value).strip():
        return default_student_t_grid(student_steps)
    grid = [float(x.strip()) for x in str(value).split(",")]
    if len(grid) != student_steps + 1:
        raise ValueError(
            f"--student-t-grid must have {student_steps + 1} values for "
            f"--student-steps={student_steps}, got {grid}"
        )
    if grid[0] != 0.0 or grid[-1] != 1.0:
        raise ValueError(f"--student-t-grid must start at 0 and end at 1, got {grid}")
    if any(grid[i] >= grid[i + 1] for i in range(len(grid) - 1)):
        raise ValueError(f"--student-t-grid must be strictly increasing, got {grid}")
    return grid


def student_t_grid_tensor(grid: Sequence[float], ref: torch.Tensor) -> torch.Tensor:
    return torch.tensor(grid, device=ref.device, dtype=ref.dtype)
...
def student_trajectory(
    ...
    t_grid = student_t_grid_tensor(student_t_grid, z)
    x = z
    states = [x]
    velocities = []
    for i in range(len(student_t_grid) - 1):
        r = t_grid[i].expand(z.shape[0])
        t = t_grid[i + 1].expand(z.shape[0])
        v = student.decoder.estimator(
            x, mask, mu, t, spks, None, streaming=streaming, r=r
        )
        dt = t_grid[i + 1] - t_grid[i]
        x = x + dt * v
        states.append(x)
...
def teacher_boundaries_at_grid(
    states: torch.Tensor,
    teacher_steps: int,
    student_t_grid: Sequence[float],
) -> List[torch.Tensor]:
    """Sample teacher trajectory states at student grid times (nearest Euler knot)."""
    bounds = []
    for tau in student_t_grid:
        idx = int(round(float(tau) * teacher_steps))
        idx = max(0, min(teacher_steps, idx))
        bounds.append(states[idx])
    return bounds
...
def teacher_boundaries(states: torch.Tensor, teacher_steps: int, student_steps: int) -> List[torch.Tensor]:
    ...
    return teacher_boundaries_at_grid(states, teacher_steps, default_student_t_grid(student_steps))
...
def distill_loss(
    ...
    with torch.no_grad():
        ...
        t_bounds = teacher_boundaries_at_grid(
            t_states, args.teacher_steps, args.student_t_grid
        )
    ...
    mean_flow_terms = []
    student_decoder = unwrap_ddp(student).decoder
    for i in range(args.student_steps):
        r_val = args.student_t_grid[i]
        t_val = args.student_t_grid[i + 1]
        dt_i = t_val - r_val
        ...
        if args.kv_cache_distill and args.decoder_streaming:
            ...
            pred_v = (x_next - x_at) / dt_i
        else:
            r_i = torch.full((x.shape[0],), r_val, device=device, dtype=mu_student.dtype)
...
def save_student_checkpoint(student, output_dir: Path, args, step: int, metrics: dict,
                            optimizer=None, scaler=None) -> Path:
    ...
    metadata = DistillMetadata(
        ...
        teacher_steps=args.teacher_steps,
        student_t_grid=list(args.student_t_grid),
        cleaner=args.cleaner,
        manifest=str(args.manifest),
        target="lits-vocos-24k-ceshi decoder integral-velocity 2-step",
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--val-manifest", type=Path, default=None,
                        help="Validation manifest. If provided, val loss is computed every --val-every steps.")
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Student/inference time grid as comma-separated floats, e.g. "0,0.6875,1.0" '
        "(VSFM coasting for 2 steps). Defaults to uniform linspace. Teacher boundary "
        "states are sampled at the same tau values.",
    )
    ...
    parser.add_argument(
        ...
        default=True,
        help="Student ODE uses chunked KV-cache path (matches causal_kv inference). "
        "Requires decoder_streaming=true.",
    )
    parser.add_argument(
        "--distill-chunk-size",
        type=int,
        default=100,
        help="Mel chunk size for KV-cache distillation (match inference CHUNK_SIZE).",
    )
    parser.add_argument(
        "--pre-lookahead-len",
        type=int,
        default=3,
        help="Decoder pre-lookahead frames per chunk (match inference).",
    )
    parser.add_argument(
        "--decoder-left-frames",
        type=int,
        default=20,
        help="Frame-level left context for student decoder (match inference decoder_left_frames).",
    )
    ...
    parser.add_argument("--val-every", type=int, default=500,
                        help="Run validation every N steps (only if --val-manifest is set).")
    parser.add_argument("--val-batches", type=int, default=50,
                        help="Number of batches to evaluate during each validation pass.")
    parser.add_argument("--dist-backend", choices=["gloo", "nccl"], default="gloo")
    parser.add_argument("--resume", type=Path, default=None,
                        help="Path to a student checkpoint to resume from. "
                             "Restores model weights, optimizer state, and continues from saved step.")
    return parser
...
def main() -> None:
    ...
    try:
        ...
        if rank == 0:
            ...
            print(f"student_t_grid={args.student_t_grid}", flush=True)
            print(streaming_config_summary(args), flush=True)
            print(
                f"kv_cache_distill={args.kv_cache_distill} "
                f"chunk_size={args.distill_chunk_size} "
                f"decoder_left_frames={args.decoder_left_frames} "
                f"pre_lookahead_len={args.pre_lookahead_len}",
                flush=True,
            )
        if distributed:
            dist.barrier()
        ...
        metadata = {
            ...
            "teacher_kv_cache_aligned": bool(args.kv_cache_distill and args.decoder_streaming),
            "decoder_streaming": args.decoder_streaming,
            "kv_cache_distill": args.kv_cache_distill,
        ...
        if rank == 0:
            ...
            log_file.write(f"\n--- resumed from step {start_step} ---\n" if start_step > 0
                           else f"--- training started ---\n")
            log_file.flush()

        pbar = tqdm(
            range(start_step + 1, end_step + 1),
            desc="distill",
            initial=0,
        ...
        for step in pbar:
            ...
            if rank == 0:
                ...
                postfix_dict = {
                    ...
                    "ep": f"{metrics['loss_endpoint']:.4f}",
                    "mf": f"{metrics['loss_mean_flow']:.4f}",
            ...
            if step % args.save_every == 0:
                if rank == 0:
                    ...
                    log_file.write(f"saved {ckpt_path}\n")
                    log_file.flush()
                if distributed:
                    dist.barrier()
            ...
            if val_loader is not None and step % args.val_every == 0:
                ...
                if val_count > 0:
                    ...
                    if rank == 0:
                        ...
                        val_line = (
                            "  [val] step={step} loss={loss_total:.6f} endpoint={loss_endpoint:.6f} "
                            "traj={loss_trajectory:.6f} mean_flow={loss_mean_flow:.6f}".format(
                                step=step, **val_metrics_avg
                            )
                        )
                        tqdm.write(val_line)
        ...
        if rank == 0:
            ckpt_path = save_student_checkpoint(student, args.output_dir, args, end_step, last_metrics,
                                               optimizer=optimizer, scaler=scaler)
            writer.close()
            done_msg = f"done; final checkpoint: {ckpt_path}"
            tqdm.write(done_msg)
